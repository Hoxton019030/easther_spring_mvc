...
@WebServlet("/ch04/ex00/updateMember.do")
...
//		MemberService memberService = new MemberServiceImpl_Jdbc();  // JDBC
		...
		if (modify.equalsIgnoreCase("DELETE")) {
			...
			if (!errorMsg.isEmpty()) {
				RequestDispatcher rd = request.getRequestDispatcher("/ch04/ex00/updateMember.jsp");
		...
		response.sendRedirect(request.getContextPath() + "/ch04/ex00/queryAllMembers.do");
