...
import ch04.ex00.model.Member;
import ch04.ex00.service.MemberService;
import ch04.ex00.service.impl.MemberServiceImpl_Hibernate;

@WebServlet("/ch04/ex00/registerMember.do")
public class RegisterMemberServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (!errorMsg.isEmpty()) {
			RequestDispatcher rd = request.getRequestDispatcher("/ch04/ex00/registerMember.jsp");
...
//		MemberService ms = new MemberServiceImpl_Jdbc();  // JDBC 
		...
		if (errorMsg.isEmpty()) {
			response.sendRedirect(request.getContextPath() + "/ch04/ex00/registerSuccess.jsp");  
			return;
		} else {
			RequestDispatcher rd = request.getRequestDispatcher("/ch04/ex00/registerMember.jsp");
			rd.forward(request, response);
